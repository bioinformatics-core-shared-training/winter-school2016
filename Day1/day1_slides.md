...
The materials for this session are available from the Data Carpentry website ([here](http://www.datacarpentry.org/R-ecology-lesson/)).
...
Make sure you have the `surveys` data in the right format:
...
We can use the `plot()` function for this, with the values for the `x` and `y` axis.
...
What do these other arguments do? 
