...
boxplot(chickwts$weight ~ chickwts$feed, xlab="Chickweight (g)", horizontal=TRUE, las=1) ## ANSWER ##
# Extending whiskers to the data extremes: ## ANSWER ##
boxplot(chickwts$weight ~ chickwts$feed, ylab="Chickweight (g)", range=0) ## ANSWER ##
```
...
barplot(by.feed.type$mean, names.arg = by.feed.type$feed, ylab="Mean weight (g)", ## ANSWER ##
        col=feed.cols) # Changing the bar colour. ## ANSWER ##
barplot(by.feed.type$mean, names.arg = by.feed.type$feed, xlab="Mean weight (g)", ## ANSWER ##
        horiz=TRUE, las=1) # Using horizontal bars. ## ANSWER ##
```
...
```{r}
# Put some code here!
by.tree <- Orange %>% ## ANSWER ##
  group_by(Tree) %>% ## ANSWER ##
  summarise_each(funs(min = min(.), ## ANSWER ##
                      q1 = quantile(., p = 0.25), ## ANSWER ##
                      median = quantile(., p = 0.5), ## ANSWER ##
                      mean = mean(.), ## ANSWER ##
                      q3 = quantile(., p = 0.75), ## ANSWER ##
                      max = max(.)), ## ANSWER ##
                 age, circumference) ## ANSWER ##
```
