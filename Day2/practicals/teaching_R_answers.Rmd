...
split(colour, fruit) # Splits colours by the fruit 
split(fruit, colour) # Split fruits by the colour 
```
...
boxplot(by.feed.type, ylab="Chickweight (g)", col=feed.cols) 
# Making the boxplots horizontal: 
boxplot(by.feed.type, xlab="Chickweight (g)", horizontal=TRUE, las=1) 
# Extending whiskers to the data extremes: 
boxplot(by.feed.type, ylab="Chickweight (g)", range=0) 
```
...
```{r}
length(by.feed.type) # Gets the length of by.feed.type (number of elements) 
lengths(by.feed.type) # Gets the length of each element of by.feed.type 
```
...
```{r}
by.tree <- split(Orange[,-1], Orange$Tree) 
lapply(by.tree, summary) 
```
